...
object MusicCardFeature {
    ...
    // 搜歌 API（Key 从 CF Worker 动态获取，不下发）
    ...
    private fun apiKey(): String {
        ...
        if (!_workerHealthy) {
            try {
                ...
                hc.connectTimeout = 3000; hc.readTimeout = 3000
        ...
        return try {
            ...
            mac.init(javax.crypto.spec.SecretKeySpec(SIGN_SECRET.toByteArray(Charsets.UTF_8), "HmacSHA256"))
            val sign = android.util.Base64.encodeToString(mac.doFinal("${ts}getkey".toByteArray(Charsets.UTF_8)), android.util.Base64.NO_WRAP)
