...
class HookEntry : IXposedHookLoadPackage, IXposedHookZygoteInit {
    ...
    private fun injectPlusMenuItem(content: View) {
        try {
            ...
            // 只处理加号菜单：contentView 类名为 db5.c5（避免误伤其他 PopupWindow）
            if (!content.javaClass.name.contains("db5.c5")) return

            // 找到菜单项容器：包含多个 m7g 子项的那个 ViewGroup
            val mItemId = ctx.resources.getIdentifier("m7g", "id", "com.tencent.mm")
            ...
            val obcId = ctx.resources.getIdentifier("obc", "id", "com.tencent.mm")
            val h5nId = ctx.resources.getIdentifier("h5n", "id", "com.tencent.mm")
            ...
            if (container is android.widget.ListView) {
                // 【临时诊断】记录注入前 ListView 状态
                val lpH = container.layoutParams?.height ?: -999
                log("加号菜单[诊断-注入前]: container=${container.javaClass.name} 高=${container.height} lpH=$lpH childCount=${container.childCount} footerCount=${container.footerViewsCount} adapterCount=${container.adapter?.count}")
                container.addFooterView(item)
                log("加号菜单：WEX 设置项已 addFooterView 到 ListView")
                container.post {
                    val lpH2 = container.layoutParams?.height ?: -999
                    log("加号菜单[诊断-注入后]: 高=${container.height} lpH=$lpH2 childCount=${container.childCount} footerCount=${container.footerViewsCount} adapterCount=${container.adapter?.count}")
                }
            } else {
...
// 在视图树中按 id 查找第一个匹配的 View（BFS）
    ...
    // 【临时诊断】递归打印视图树：类名 / id / 尺寸 / 可见性 / 文本
    private fun dumpTree(v: View, depth: Int) {
        try {
            ...
            val text = if (v is android.widget.TextView) " text=\"${v.text}\"" else ""
            log("$indent${v.javaClass.name} id=$id ${v.width}x${v.height} vis=$vis$text")
            // 【临时诊断】遇到 RecyclerView，打印其 adapter / layoutManager / item 数量，用于 B2 定位
            if (v.javaClass.name.contains("RecyclerView")) {
                try {
                    val adapter = XposedHelpers.callMethod(v, "getAdapter")
                    val lm = XposedHelpers.callMethod(v, "getLayoutManager")
                    val count = if (adapter != null) XposedHelpers.callMethod(adapter, "getItemCount") else -1
                    log("$indent  >> ADAPTER=${adapter?.javaClass?.name} LAYOUTMGR=${lm?.javaClass?.name} itemCount=$count")
                } catch (e: Exception) {
                    log("$indent  >> RecyclerView 信息读取失败: ${e.message}")
                }
            }
            if (v is ViewGroup) {
                for (i in 0 until v.childCount) {
                    v.getChildAt(i)?.let { dumpTree(it, depth + 1) }
                }
            }
        } catch (_: Exception) {}
    }
    ...
    private fun applyStyle(tabView: ViewGroup) {
        ...
        // 【临时诊断】打印底栏层级：定位"胶囊外那圈方"来自哪一层
        try {
            log("===== 底栏诊断开始 =====")
            log("tabView=${tabView.javaClass.name} 尺寸=${tabView.width}x${tabView.height} bg=${tabView.background}")
            for (i in 0 until tabView.childCount) {
                val c = tabView.getChildAt(i)
                log("  tabView.child[$i]=${c.javaClass.name} 尺寸=${c.width}x${c.height} bg=${c.background}")
            }
            var pp: Any? = tabView.parent
            var lvl = 1
            while (pp is ViewGroup && lvl <= 3) {
                log("  父层[$lvl]=${pp.javaClass.name} 尺寸=${pp.width}x${pp.height} bg=${pp.background}")
                pp = pp.parent
                lvl++
            }
            log("===== 底栏诊断结束 =====")
